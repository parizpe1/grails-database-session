...
class DatabaseSessionGrailsPlugin extends Plugin {
    ...
    def title = "Database Session" // Headline display name of the plugin
...
//    def scm = [ url: "http://svn.codehaus.org/grails-plugins/" ]
    ...
    Closure doWithSpring() {
        { ->
            if (!isConfigEnabled()) {
                println "Not enabling database session plugin"
                return
            }
    ...
    private boolean isConfigEnabled() {
        def enabled = Holders.getFlatConfig()["grails.plugin.databasesession.enabled"]
